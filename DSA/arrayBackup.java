...
// Q.2.1 find MINIMUM element in an array
...
// for (int i = 1; i < arr.length; i++) {
// if (arr[i] != arr[j]) {
// j++; // Move j to the next position for a unique element
// arr[j] = arr[i]; // Place the unique element at the j-th position
// }
// }
...
// Another method but logic is not working
...
// ho , Max sabse door point batata hai jahan tak aap jaa sakte ho,
// int jumps =0 ; // The number of jumps lage to reach to that index.
// int maxRange=0; // maxRange yeh decide karta hai ki aapko kab next jump karna
// hai.

// for (int i = 0; i < arr.length; i++) {
// MaxReach=Math.max(MaxReach,i+arr[i]); // comparing from the previous one
// if(maxRange==i){ // iska matlab hai ki aap ab tak ke jumps ke saath maxRange
// tak pahunch gaye ho.
// maxRange=MaxReach;
// jumps++;
// if(maxRange>=n-1){
...
// // In summary, j keeps track of the position where the next unique element
// should be placed,
// // and i is used in the loop to access and print the unique elements stored
// in the array up to j.
// for (int i = 0; i < j; i++) {
// System.out.print(arr[i] + " ");
// }
// System.out.println();
...
// }
// else {
// // Step 3: Find the largest index `j` greater than `idx` such that nums[j] >
// nums[idx]
// int j;
// for (j = n - 1; j > idx; j--) {
// if (nums[j] > nums[idx]) {
...
// }
// --------------------------------------------------------------------------------------------------------------------//
// Q. 17.1 Best Time to Buy and Sell Stock II ; - you can buy ans sell stock
// multiple times
...
// int[] left = new int[n]; // left[i] stores the maximum profit achievable from
// day 0 to day i.
// int[] right = new int[n];
...
// for (int i = 0; i < arr.length; i++) {
// int x = target - arr[i];
// if (map.containsKey(x)) { // x hash map me phle se presnt hai ?
// ans += map.get(x); // ans me add kar denge jitni baar bhi occur hua tha utni
// baar increment kar-do
// }
// if (map.containsKey(arr[i])) { // arr[i] ko hash me daal do or incrememnt
// karo count ko
...
// array(0), and i starts where the positives begin.
// swap(arr, k, i); // So, the swap function alternates a positive number with a
// negative number, starting from index 0.
// i = i + 1;
// k = k + 2;
// }
// } // rearrange
...
// // If the current sum is 0 or has been seen before, a subarray with sum 0
// exists
...
// int right = n - 1; // Pointer to the right side of the array

// for (int i = 0; i <= right; i++) {
// if (arr[i] <a) {
// // Swap current element with the left pointer if it's less than a
// swap(arr, left, i);
// left++;
// } else if (arr[i] > b) {
// // Swap current element with the right pointer if it's greater than b
// swap(arr, right, i);
// right--;
// i--; // Decrement i to re-evaluate the swapped element at this position
// }
// }
// }
